# -*- coding: utf-8 -*-
"""
Created on Tue Sep  2 08:25:46 2025
Modified to apply FiLM only on the final MLP head (two-layer MLP),
added z-score normalization per task with reverse transformation at evaluation,
and integrated precomputed Mordred descriptors after FiLM layer.
...
# ------------------------------
# Compute Mordred descriptors once
# ------------------------------
def compute_mordred_for_data(data_list, desc_dim=64):
    from mordred import Calculator, descriptors
    import numpy as np
    import torch
    ...
    for data in data_list:
        try:
            ...
            if mol is None:
                desc_matrix.append(np.zeros(desc_dim))
                continue
            desc_vals = calc(mol).fill_missing(0).asdict().values()
            desc_tensor = np.array(list(desc_vals), dtype=float)
            if desc_tensor.size > desc_dim:
                desc_tensor = desc_tensor[:desc_dim]  # 截断前 desc_dim 个特征
            elif desc_tensor.size < desc_dim:
                # 不够补零
                desc_tensor = np.pad(desc_tensor, (0, desc_dim - desc_tensor.size))
            desc_matrix.append(desc_tensor)
        except Exception as e:
            print(f"[WARN] Mordred failed for {data.smiles}: {e}")
            desc_matrix.append(np.zeros(desc_dim))

    # 转为矩阵进行 z-score 标准化
    desc_matrix = np.stack(desc_matrix)
    mean = desc_matrix.mean(axis=0, keepdims=True)
    std = desc_matrix.std(axis=0, keepdims=True)
    std[std < 1e-6] = 1.0
    desc_matrix = (desc_matrix - mean) / std

    # 回写到每个 Data 对象
    for i, data in enumerate(data_list):
        data.desc = torch.tensor(desc_matrix[i], dtype=torch.float)
...
# ------------------------------
# Mordred descriptor encoder (MLP)
# ------------------------------
class MordredEncoder(nn.Module):
...
# ------------------------------
# MLP with FiLM + Mordred
# ------------------------------
class MLPWithFiLMAndDesc(nn.Module):
    ...
    def forward(self, x, task_emb, desc_feat=None):
        gamma1, beta1 = self.film_generator_1(task_emb)
        x = gamma1 * x + beta1
        x = F.relu(self.fc1(x))
        x = self.dropout(x)
        if desc_feat is not None:
            desc_encoded = self.desc_encoder(desc_feat)
            x = x + desc_encoded  # GNN+FiLM output + descriptor
...
class GINRegressor(nn.Module):
    ...
    def forward(self, data: Data):
        ...
        # Stack descriptor features
        desc_feat = torch.stack([d.desc for d in data.to_data_list()], dim=0).to(batch.device)
        out = self.head(g, task_emb, desc_feat)
        return out.view(-1)
...
def main():
    ...
    for task_id, csv in enumerate(data_files):
        ...
        ds = compute_mordred_for_data(ds)  # 预计算描述符
        all_data.extend(ds)
    ...
    model = GINRegressor(
        ...
        layers=4,
        dropout=0.02,
        desc_dim = all_data[0].desc.shape[0]
    ).to(device)
    ...
    test_metrics = task_evaluate(model, test_loader, device, task_stats, num_tasks=len(data_files))
